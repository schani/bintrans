...
let rec normalize_expr expr =
  let rec normalize_step expr =
	...
	Binary (IntAdd, c, x) when (is_const c) && (not (is_const x)) ->
	  (* c+x -> x+c *)
	  Binary (IntAdd, x, c)
      | Binary (IntSub, c, x) when (is_const c) && (not (is_const x)) ->
	  (* c-x -> (-x)+c *)
	  Binary (IntAdd, Unary (IntNeg, x), c)
      | Binary (IntSub, x, c) when is_const c ->
	  (* x-c -> x+[-c] *)
	  Binary (IntAdd, x, cfold_expr [] (Unary (IntNeg, c)))
      | Binary (IntAdd, Binary (IntAdd, x, c), y) when (not (is_const x)) && (is_const c) && (not (is_const y)) ->
	  (* (x+c)+y -> (x+y)+c *)
	  Binary (IntAdd, Binary (IntAdd, x, y), c)
      | Binary (IntSub, Binary (IntAdd, x, c), y) when (not (is_const x)) && (is_const c) && (not (is_const y)) ->
	  (* (x+c)-y -> (x-y)+c *)
	  Binary (IntAdd, Binary (IntSub, x, y), c)
      | Binary (IntAdd, Binary (IntAdd, x, c), d) when (not (is_const x)) && (is_const c) && (is_const d) ->
	  (* (x+c)+d -> x+[c+d] *)
	  Binary (IntAdd, x, cfold_expr [] (Binary (IntAdd, c, d)))
      | Binary (IntSub, Binary (IntAdd, x, c), d) when (not (is_const x)) && (is_const c) && (is_const d) ->
	  (* (x+c)-d -> x+[c-d] *)
	  Binary (IntAdd, x, cfold_expr [] (Binary (IntSub, c, d)))
      | Binary (IntAdd, x, Binary (IntAdd, y, c)) when (not (is_const x)) && (not (is_const y)) && (is_const c) ->
	  (* x+(y+c) -> (x+y)+c *)
	  Binary (IntAdd, Binary (IntAdd, x, y), c)
      | Binary (IntSub, x, Binary (IntAdd, y, c)) when (not (is_const x)) && (not (is_const y)) && (is_const c) ->
	  (* x-(y+c) -> (x-y)+[-c] *)
	  Binary (IntAdd, Binary (IntSub, x, y), cfold_expr [] (Unary (IntNeg, c)))
      | _ -> nexpr
